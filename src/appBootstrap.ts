...
if (!existsSync(env)) {
  throw `env file does not exist: ${env}`;
}
dotenv.config({
  path: env
});
...
const app = new App({
  ...
  authControllerConfig: {
    ...
    emailTemplate: JSON.parse(
      readFileSync('./env/password_reset_mail_template.json').toString('UTF-8')
    ),
    ...
    authRelatedLimiterByEmailAndIpPoints: Number(
      process.env.AUTH_RELATED_LIMITER_BY_EMAIL_AND_IP_POINT!
    ),
    authRelatedLimiterByEmailAndIpDuration: Number(
      process.env.AUTH_RELATED_LIMITER_BY_EMAIL_AND_IP_DURATION!
    )
  },
